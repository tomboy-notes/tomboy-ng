...
빌드 일자: 2026-09-02 (GTK4 `onion2` 는 같은 날 20:08 재빌드)
...
| | 값 |
| --- | --- |
| 업스트림 버전 (`package/version`) | `0.42` |
| 로컬 접미사 (`LOCAL_VERSION_SUFFIX`) | GTK4 `onion2`, Qt5 `onion1` |
| 패키지/앱 버전 | `0.42+onion2` (GTK4), `0.42+onion1` (Qt5) |

`onion2` 는 LCL GTK4 의 close-request 수정(`lcl-gtk4 4.4+dfsg-4`, 아래 Toolchain)을
반영한 재빌드다. Qt5 는 그 수정의 영향을 받지 않으므로 `onion1` 그대로 둔다.

`Version_string`(`source/cli.pas:61`)이 컴파일 시점 `%TOMBOY_NG_VER`를 읽으므로
About 다이얼로그와 `--version` 출력에도 `0.42+onion1`이 표시된다.
...
| 저장소 | 경로 | 브랜치 | 커밋 |
| --- | --- | --- | --- |
| tomboy-ng | `/mnt/STORAGE16T/Workspace_STORAGE16T/tomboy-ng` | `gtk4-build-editor-fallback` | `c9cb258` (onion2) / `a63001e` + 로컬 변경 (onion1) |
| KControls | `/mnt/STORAGE16T/Workspace_STORAGE16T/KControls` (`../KControls`) | `integration-fixes` | `74d3257` (onion2) / `8e41d45` (onion1) — 둘 사이 차이는 문서·테스트 스크립트 경로뿐 |
...
| --- | --- |
| FPC | 3.2.2 |
| Lazarus | `/usr/lib/lazarus/4.4` (`lazarus-src-4.4` 4.4+dfsg-4; onion1 은 dfsg-3) |
| LCL GTK4 | `lcl-gtk4` 4.4+dfsg-4 (로컬 빌드 패키지, `../LCL_GTK4/deb_build/4.4`) |
| LCL Qt5 | `lcl` 4.4+dfsg-3 (onion1 빌드 당시) |
...
`objdump -d .../lcl/units/x86_64-linux/gtk4/gtk4widgets.o` 에서 `GTK4CLOSEQUERY` 가
`HANDLEALLOCATED` 를 호출하는지로 확인할 수 있다.
...
```bash
WIDGETSET=gtk4 LOCAL_VERSION_SUFFIX=onion2 ./build_widgetset_clean.sh
...
```bash
cd package
LOCAL_VERSION_SUFFIX=onion2 bash package.bash /usr/lib/lazarus/4.4 DebOnly:ReleaseGTK4
...
| 패키지 | Depends | 비고 |
| --- | --- | --- |
| `tomboy-ng_0.42+onion2-0_amd64Gtk4.deb` | `libgtk-4-1 (>= 4.6), libnotify4 (>= 0.7), libc6 (>= 2.34)` | 현재 GTK4 패키지 |
| `tomboy-ng_0.42+onion1-0_amd64Qt5.deb` | `libqt5pas1 (>= 2.15), libc6 (>= 2.34), libnotify-bin` | 현재 Qt5 패키지 |
| `tomboy-ng_0.42+onion1-0_amd64Gtk4.deb` | 위와 동일 | **폐기 대상.** close-request 버그가 있는 dfsg-3 LCL 로 빌드됨 |
...
- LCL Qt5 콤보 버그 B7 / B8 / B9 는 미수정 상태로 남는다
  (`LCL_GTK4/TODO.md`).
- `dch` 가 `0.42+onion1` 을 Debian native 버전으로 보고 경고를 낸다.
