...
uses {$ifdef LCL}laz2_DOM, laz2_XMLRead, LazFileUtils{$else}DOM, XMLRead, LazFileUtils{$endif};
...
begin
    ...
    if not DeleteFile(FullFileName) then begin
    	...
    	if not FileExistsUTF8(FullFileName) then
            {$ifdef LCL}Debugln{$else}writeln{$endif}('But, FileExists says its gone, proceed !')
        else begin
